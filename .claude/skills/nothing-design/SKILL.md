...
A senior product designer's toolkit grounded in Nothing's core identity: **transparency, industrial honesty, and typographic precision.** Informed by Swiss typography, Braun/Teenage Engineering industrial design, IBM mainframe-era dot-matrix aesthetics, and Nothing's own evolving design language (Phone 1 → Phone 4a). Monochromatic by default, with intentional color as a material property — not decoration.
...
- **Transparency is the philosophy.** Nothing's founding principle: expose internal structure. Circuits, grids, hierarchy itself become the ornament. In UI, this means: show the data, show the system, show the logic. Nothing to hide.
- **Subtract, don't add.** Every element must earn its pixel. Default to removal. If something can be communicated with spacing alone, don't add a border.
- **Engineering-led honesty.** Products and interfaces should feel "led by engineering" — functional, purposeful, structurally legible. A toggle looks like a physical switch. A gauge looks like an instrument. No cosmetic dishonesty.
- **Type does the heavy lifting.** Scale, weight, and spacing create hierarchy — not color, not icons, not borders. The dot-matrix motif (NDot / Doto) is the brand's visual signature.
- **Both modes are first-class.** Dark mode: OLED instrument panel in a dark room. Light mode: printed technical manual on off-white paper. Neither is "derived" — both get full design attention. Ask the user which mode to start with.

### 1.2 The Nothing Identity

Nothing's design draws from specific historical sources:

| Source | What it contributes |
|--------|---------------------|
| **IBM mainframe era (1980s)** | Dot-matrix typography, monospaced data readouts, grid-based displays |
| **Braun (Dieter Rams)** | "Less but better," form follows function, honest materials |
| **Teenage Engineering** | Playful precision, exposed mechanics, product-as-instrument |
| **Swiss typography** | Grid discipline, hierarchy through scale and weight, asymmetric composition |
| **Nothing's own evolution** | Glyph Interface, transparent materials, color-through-material layering |

### 1.3 Transparency in Practice

In hardware, Nothing exposes PCBs and coils through clear glass. In UI, the equivalent is:

- **Expose the grid.** Dot-grid backgrounds, visible alignment, structural regularity.
- **Expose the data.** Numbers, states, and values ARE the visual. No illustrations needed.
- **Expose the hierarchy.** The three-layer system (Section 2.1) is self-evident — the user sees how information is organized without needing cues.
- **Layered depth.** Like Nothing's tinted glass over resin, use subtle surface elevation (`--surface` → `--surface-raised`) to create depth without shadows.
...
**Color as material (Nothing's evolution):** Starting with Phone (2a) and (4a), Nothing uses color not as paint but as a property of layered transparent materials — tinted resin under glass creates depth and shifts with light. In UI, this translates to: if color is used, it should feel like it emerges from the surface, not painted on. Use `color-mix()`, transparency, and subtle tints over dark/light surfaces.
...
7. **The back of the phone is the front.** Nothing's transparent backs are not gimmick — they express that the interesting part is the engineering. In UI: show the system, show the structure, show the work.

### 2.9 The Glyph Principle

Nothing's Glyph Interface (LED patterns on the back) and Glyph Matrix (489-LED dot-matrix display on Phone 3) represent a core design idea: **communicate with minimal resolution.**

In UI, apply the Glyph principle:
- **Low-fidelity can be high-impact.** A single red dot, a segmented bar, a monospaced number — these carry more meaning than a detailed illustration.
- **Dot-matrix as texture.** Use the dot-grid motif for backgrounds, loading states, empty states, and decorative moments.
- **Light as information.** In Glyph, LED brightness and animation encode meaning (timer countdown, notification type). In UI: use opacity, pulsing (`blink` keyframes), and subtle animation to encode state.
- **Restraint in animation.** Glyph animations are discrete, not fluid. Segmented, not continuous. Apply the same: prefer stepped transitions, discrete progress bars, and mechanical motion.

### 2.10 Visual Variety in Data-Dense Screens
...
- No decorative color. Color must be functional (status, accent, interactive) or material (emerging from surface layers).
- No "friendly" rounded bubbly UI. Technical precision, not consumer softness.
...
4. **Compose** — apply craft rules (Sections 2.1–2.10)
...
7. **Apply the Glyph Principle** — where can resolution be reduced for greater impact?
...
- **`references/tokens.md`** — Fonts (including NDot/Doto lineage), type scale, color system (dark + light), spacing scale, grid, motion, iconography, dot-matrix motif
