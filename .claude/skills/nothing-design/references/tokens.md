...
**Why these fonts:** Doto = variable dot-matrix (closest to NDot 57). Space Grotesk + Space Mono by Colophon Foundry — the same foundry that created Nothing's proprietary typefaces (NDot 57, NType 82, NType 82 Mono). Shared design DNA.

### Font Lineage

Nothing's official typefaces (not publicly available) and their open equivalents used in this system:

| Nothing official | Role | Open equivalent | Notes |
|-----------------|------|-----------------|-------|
| **NDot 57** | Display, dot-matrix | **Doto** (Google Fonts) | Variable dot-size axis. IBM 1980s mainframe-era dot-matrix inspiration. |
| **NType 82** | Body, UI | **Space Grotesk** | Same foundry (Colophon). Geometric sans, humanist warmth. |
| **NType 82 Mono** | Data, labels | **Space Mono** | Same foundry. Monospaced companion. |
| **LL Lettera Mono** | Website accent | **Space Mono** | Close match in character. |
...
### Color-as-Material (Nothing's Evolution)

Starting with Phone (2a) and (4a), Nothing uses color not as paint but as a physical property of layered transparent materials: tinted resin under glass creates depth that shifts with ambient light (similar to iMac G3's translucent colored plastic). In UI:

- Use `color-mix()` and `rgba()` transparency to create depth over surfaces.
- Tints should feel like they **emerge from** the surface, not applied **on top of** it.
- If adding a brand color, apply it as a subtle surface tint (`--accent-subtle`) rather than a solid fill.
- The mid-frame color principle: accents near edges can be slightly more saturated than large surface areas.
